                          ...
                          CUBReduceFunc cub_reduce_func)
{
  ...
  RAFT_CUDA_TRY(cub_reduce_func(nullptr, temp_storage_bytes, in, out, n_segments, offsets, offsets + 1, stream));
  d_temp_storage.resize(temp_storage_bytes, stream);

  RAFT_CUDA_TRY(
    cub_reduce_func(d_temp_storage.data(), temp_storage_bytes, in, out, n_segments, offsets, offsets + 1, stream));
}
